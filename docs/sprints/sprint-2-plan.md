...
- [x] [#8](https://github.com/KerdanetYvan/urbanflow-mobility/issues/8) (Dev FE) — Affichage cartographique du trajet — fixtures OTP de test relocalisées Lyon → Rennes au passage, voir PR #89
- [x] [#36](https://github.com/KerdanetYvan/urbanflow-mobility/issues/36) (Dev FE) — Écran de résultats d'itinéraires — badges qualitatifs (docs/specs/f3-scoring-perturbations.md §2.2) reportés à #16 (Sprint 3), décision prise en session le 2026-08-03
- [x] [#9](https://github.com/KerdanetYvan/urbanflow-mobility/issues/9) (Dev FE) — Géolocalisation temps réel de l'utilisateur — pré-remplissage de l'origine de recherche avec la position reporté à #93 (Stretch)
- [x] [#68](https://github.com/KerdanetYvan/urbanflow-mobility/issues/68) (Dev BE) — Remplacer "mobilité réduite" par des préférences d'accessibilité détaillées — `reducedMobility`/`maxWalkingDistanceMeters`/`maxTransfers` remplacés par `accessibilityPreferences` (tableau extensible, décision prise en session le 2026-08-06 : pas de champ libre ni de seuil numérique, incompatibles avec un futur score pondéré). A introduit au passage l'infrastructure de migrations TypeORM (absente jusqu'ici, `synchronize` géré) pour satisfaire le critère de migration des données existantes — voir `backend/README.md`. Frontend (`ProfilPage.tsx`) pas encore mis à jour, cassé jusqu'à #69 juste après.
- [x] [#69](https://github.com/KerdanetYvan/urbanflow-mobility/issues/69) (Dev FE) — Formulaire de profil : préférences d'accessibilité détaillées
- [x] [#66](https://github.com/KerdanetYvan/urbanflow-mobility/issues/66) (Dev BE) — Étendre les modes de transport en commun (bus/métro/tram/train-TER) — `public_transport` remplacé par `bus`/`tram`/`metro`/`train_ter`, migration de backfill vers `bus` par defaut. Frontend (`TRANSPORT_MODES`/`ProfilPage.tsx`) pas encore mis à jour, cassé jusqu'à #67 juste après.
...
- [x] [#73](https://github.com/KerdanetYvan/urbanflow-mobility/issues/73) (Dev FE) — Refonte de la disposition mobile/desktop — icônes de nav, correctif de centrage desktop, et fusion RecherchePage+ResultatsPage en un seul écran (`RecherchePage` + `RecherchePageResults`, machine à états) ; `/resultats` redirige vers `/recherche`
