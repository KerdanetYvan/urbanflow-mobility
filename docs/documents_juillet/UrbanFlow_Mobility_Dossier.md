...
| Tests unitaires | Jest | Tests du backend NestJS et des composants React |
...
|---|---|---|
| Couverture de tests | Jest | À chaque exécution de la CI |
...
Pour les besoins statistiques évoqués en [partie 2.4](#24-enjeux-métiers-et-objectifs-économiques) (orienter les décisions d'infrastructure de la métropole), les données utilisées sont agrégées et anonymisées : la collectivité peut savoir qu'une ligne de bus est saturée aux heures de pointe sans que cela remonte à un usager identifiable. L'hébergement en France ou en Europe, déjà justifié en [partie 3.9](#39-benchmark-des-hébergeurs-cloud), complète cette conformité en évitant tout transfert de données hors UE.
...
Cette exigence dépasse le seul champ de l'interface : le planificateur d'itinéraires lui-même doit pouvoir signaler des obstacles physiques (escaliers, trottoirs dégradés) dans le calcul d'un trajet, une préférence déjà intégrée au profil de mobilité (F1) mentionnée en [partie 4.4](#44-spécifications-fonctionnelles-des-modules-principaux). *(Grille de conformité détaillée en [annexe F](#annexe-f-grille-de-conformité-wcag-21-aa).)*
