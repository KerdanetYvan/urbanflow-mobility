...
> Casquette Dev FE (+ un ajout ponctuel Dev BE, voir section 7) — issue [#129](https://github.com/KerdanetYvan/urbanflow-mobility/issues/129), Sprint 3.
...
> Étendue en session le 2026-08-18 (section 7) : couleur par ligne (GTFS `route_color`/`route_text_color`) sur les badges et sur le tracé de la carte — revient sur la décision de couleur neutre de la section 4, voir section 7 pour le raisonnement.
...
## 7. Extension — couleur de ligne (GTFS) sur les badges et la carte
...
### 7.2 Flux de données (nouveau, Dev BE)
...
### 7.3 `tripModeChips` : source commune carte + badges
...
`tripModeChips.ts` porte aussi `chipLabel(chip)`, ajoutée à la revue finale de branche comme seul et unique endroit qui construit le libellé complet d'une puce (mode + ligne, ou mode seul quand le libellé de ligne est déjà celui du mode) — réutilisée à la fois par la légende de `MapView` et par le texte accessible `modesLabel` de `RecherchePageResults.tsx`, qui construisaient auparavant chacune leur propre copie de cette règle, désormais divergentes (c'est exactement cette divergence qui causait le bug de libellé doublé "Bus Bus" décrit en 7.7).

### 7.4 `LineBadge` : fond plein
...
### 7.5 `MapView` : tracé et légende par ligne
...
- `segmentColor.spec.ts` : teste directement `getSegmentColor()` en unitaire pur plutôt que d'inspecter `pathOptions.color` d'un `Polyline` react-leaflet en jsdom (rendu SVG imperatif peu fiable à asserter en test) — couleur GTFS présente → utilisée ; absente sur un segment de transport en commun → repli sur la couleur du mode ; segment hors transport en commun → toujours la couleur du mode, couleur GTFS ignorée.
- `MapView.spec.tsx` : légende affiche un swatch par ligne avec la bonne couleur ; un segment de transport en commun sans `routeName` légende avec le libellé du mode seul, jamais doublé (ex. "Bus", pas "Bus Bus" — régression de la revue finale de branche, voir `chipLabel` en 7.3).
